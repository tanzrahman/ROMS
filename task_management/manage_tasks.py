...
def task_list(request):
    ...
    total_completed_tasks = Task.objects.filter(task_category__in=['DocumentReview', 'Document Review'], percent_completed__exact=100).count()
    total_incomplete_tasks = Task.objects.filter(task_category__in=['DocumentReview', 'Document Review'], percent_completed__lt=100).count()
    total_monthly_tasks = Task.objects.filter(task_category__in=['DocumentReview', 'Document Review'], planned_start_date__year=datetime.datetime.today().year, planned_start_date__month=datetime.datetime.today().month).count()
    ...
    if (len(filters) > 0):
        task_list = Task.objects.filter(task_category__in=['DocumentReview', 'Document Review']).filter(reduce(operator.and_, filters))
        search_summary= {
        ...
        task_list = Task.objects.filter(task_category__in=['DocumentReview', 'Document Review'])
