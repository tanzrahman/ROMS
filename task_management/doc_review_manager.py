...
def list_all_doc(request,action=None,id=None):
    as_supervisor = Task.objects.filter(task_category__in=['DocumentReview', 'Document Review'], supervisor=request.user, created_date__gt='2025-07-31')
    as_executor = Task.objects.filter(task_category__in=['DocumentReview', 'Document Review'], task_executor=request.user, created_date__gt='2025-07-31')
...
def download_committee_review(request, id):
    ...
    for each in committee_rev_cmnt:
        ...
        if(len(each.proposed_text)>12000 or len(each.original_text)>12000 or len(each.remarks)>12000):
            no_pdf = True
